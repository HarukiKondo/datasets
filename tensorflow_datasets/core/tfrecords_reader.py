...
import numpy as np
from riegeli.tensorflow.ops import riegeli_dataset_ops as riegeli_tf
...
def _get_dataset_from_filename(filename_skip_take, do_skip, do_take,
  ...
  if file_format == constants.FileFormat.TFRECORD:
    ...
    ds = riegeli_tf.RiegeliDataset(filename, buffer_size=_BUFFER_SIZE)
  else:
    raise ValueError(
...
def make_file_instructions(
    ...
    file_format: constants.FileFormat = constants.DEFAULT_FILE_FORMAT,
...
def _make_file_instructions_from_absolutes(
  ...
  for abs_instr in absolute_instructions:
    ...
    filenames = naming.filenames_for_dataset_split(
        ...
        filetype_suffix=constants.FILE_FORMAT_TO_FILE_SUFFIX.get(file_format))
...
class Reader(object):
  ...
  def __init__(self, path, example_specs, file_format):
    """Initializes Reader.
